...
function toReview(row: Record<string, unknown>): FactCandidateReviewRecord {
  return {
    ...
    snapshotContentHash: text(row.snapshot_content_hash),
    blockIndex: number(row.block_index),
    chunkIndex: number(row.chunk_index),
    blockContent: text(row.block_content),
    sourceTitle: text(row.source_title),
    sourceUrl: text(row.source_url),
    sourceType: text(row.source_type) as FactCandidateReviewRecord['sourceType'],
